  var DIRECAO_ESQUERDA = 1;
  var DIRECAO_DIREITA = 2;

  function Jogador(context, teclado, animacao) {
    this.context = context;
    this.teclado = teclado;
    this.animacao = animacao;
    this.vida = 100;
    this.x = 0;
    this.y = 0;
    this.width = 450;
    this.height = 400;
    this.direcao = DIRECAO_DIREITA;
    this.velocidadeY = 0;
    this.pulando = false;
    this.agachado = false;
    this.corVida = "green";
    this.isMoving = false;
    this.movingBack = false;

    // Código para carregar e configurar sprites
    this.spriteParado = new Image();
    this.spriteParado.src = "../img/Sprites/Idle.png";
    this.numSpritesIdle = 4;
    this.largSpriteIdle = this.spriteParado.width / this.numSpritesIdle;
    this.altSpriteIdle = this.spriteParado.height;
    this.frameIdle = 0;
    this.contadorIdle = 0;

    this.spriteCorrendo = new Image();
    this.spriteCorrendo.src = "../img/Sprites/Run.png";
    this.numSpritesCorrendo = 8;
    this.largSpriteCorrendo = this.spriteCorrendo.width / this.numSpritesCorrendo;
    this.altSpriteCorrendo = this.spriteCorrendo.height;
    this.frameCorrendo = 0;
    this.contadorCorrendo = 0;

    this.spritePulando = new Image();
    this.spritePulando.src = "../img/Sprites/Jump.png";
    this.numSpritesPulando = 2;
    this.largSpritePulando = this.spritePulando.width / this.numSpritesPulando;
    this.altSpritePulando = this.spritePulando.height;
    this.framePulando = 0;
    this.contadorPulando = 0;
  }

  Jogador.prototype = {
    atualizar: function () {
      var frameWidth, frameHeight;

      if (this.pulando) {
        frameWidth = this.largSpritePulando;
        frameHeight = this.altSpritePulando;
      } else if (this.isMoving) {
        frameWidth = this.largSpriteCorrendo;
        frameHeight = this.altSpriteCorrendo;
      } else {
        frameWidth = this.largSpriteIdle;
        frameHeight = this.altSpriteIdle;
      }

      // ESQUERDA E DIREITA
      if (this.teclado.pressionada(SETA_ESQUERDA) && this.x > 0) {
        this.x -= 5;
        this.direcao = DIRECAO_ESQUERDA;
        this.isMoving = true;
        this.movingBack = false;
      } else if (this.teclado.pressionada(SETA_DIREITA) && this.x < this.context.canvas.width - frameWidth) {
        this.direcao = DIRECAO_DIREITA;
        this.x += 5;
        this.isMoving = true;
        this.movingBack = true;
      } else {
        this.isMoving = false;
        this.movingBack = false;
      }

      // PULO E AGACHAR
      if (this.teclado.pressionada(SETA_CIMA) && !this.pulando) {
        this.velocidadeY = -12;
        this.pulando = true;
      }

      if (this.teclado.pressionada(SETA_BAIXO) && !this.pulando) {
        this.agachado = true;
      } else {
        this.agachado = false;
      }

      this.y += this.velocidadeY;
      this.velocidadeY += 0.5;

      if (this.y >= this.context.canvas.height - frameHeight) {
        this.y = this.context.canvas.height - frameHeight;
        this.velocidadeY = 0;
        this.pulando = false;
      }

      // LIMITE DE TELA
      if (this.x + frameWidth > this.context.canvas.width) {
        this.x = this.context.canvas.width - frameWidth;
      }

      if (this.x < 0) {
        this.x = 0;
      }

      // MORRER
      if (this.vida <= 0) {
        this.morrer();
      }
    },

    morrer: function () {
      console.log("Game Over");
      this.vida = 100;
      this.x = 0;
      this.y = 300;
    },

    desenhar: function () {
      var sprite, frameX, frameY = 45, frameWidth, frameHeight;

      if (this.pulando) {
        // Animação de pulo
        this.contadorPulando++;
        if (this.contadorPulando >= 5) {
          this.framePulando = (this.framePulando + 1) % this.numSpritesPulando;
          this.contadorPulando = 0;
        }
        sprite = this.spritePulando;
        frameWidth = this.largSpritePulando;
        frameHeight = this.altSpritePulando;
        frameX = this.framePulando * frameWidth;
      } else if (this.isMoving) {
        // Animação correndo
        this.contadorCorrendo++;
        if (this.contadorCorrendo >= 5) {
          this.frameCorrendo = (this.frameCorrendo + 1) % this.numSpritesCorrendo;
          this.contadorCorrendo = 0;
        }
        sprite = this.spriteCorrendo;
        frameWidth = this.largSpriteCorrendo;
        frameHeight = this.altSpriteCorrendo;
        frameX = this.frameCorrendo * frameWidth;
      } else {
        // Animação parado
        this.contadorIdle++;
        if (this.contadorIdle >= 10) {
          this.frameIdle = (this.frameIdle + 1) % this.numSpritesIdle;
          this.contadorIdle = 0;
        }
        sprite = this.spriteParado;
        frameWidth = this.largSpriteIdle;
        frameHeight = this.altSpriteIdle;
        frameX = this.frameIdle * frameWidth;
      }

      // Ajustar posX ao mudar a direção
      var posX = this.direcao === DIRECAO_DIREITA ? this.x : this.x  + frameWidth;
      var scaleX = this.direcao === DIRECAO_ESQUERDA ? -1 : 1;

      this.context.save();
      this.context.translate(posX + (scaleX === -1 ? frameWidth : 0), this.y);
      this.context.scale(scaleX, 1);
      this.context.drawImage(
        sprite,
        frameX,
        frameY,
        frameWidth,
        frameHeight,
        0,
        0,
        this.width,
        this.height
      );
      this.context.restore();

      // Desenhar barra de vida
      var textoVidaX = 16;
      var textoVidaY = this.context.canvas.height - 23;
      var barraVidaX = 15;
      var barraVidaY = this.context.canvas.height - 20;
      var barraVidaWidth = 30;
      var barraVidaHeight = 10;

      this.context.fillStyle = this.corVida;
      this.context.font = "18px Impact, fantasy";
      this.context.fillText(this.vida, textoVidaX, textoVidaY);
      this.context.fillRect(
          barraVidaX,
          barraVidaY,
          barraVidaWidth,
          barraVidaHeight
      );
      console.log(this.x)
    },
  };
